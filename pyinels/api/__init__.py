...
class Api:
    ...
    async def ping(self):
        ...
        return await(await self.loop.run_in_executor(None, self.proxy.ping))

    async def getPlcIp(self):
        """Get Ip address of PLC."""
        return await(await self.loop.run_in_executor(None, self.proxy.getPlcIP))

    async def getRooms(self):
        """List of all rooms from Connection server website."""
        return await(await self.loop.run_in_executor(None, self.proxy.getRooms))
    ...
    async def getRoomDevices(self, room_name):
        ...
        return await(await self.__roomDevicesToJson(room_name))
    ...
    async def read(self, device_ids):
        ...
        return await(await self.__readDeviceData(device_ids))
    ...
    async def __writeValues(self, command):
        ...
        await(self.loop.run_in_executor(None, self.proxy.writeValues, command))
